...
import www.ilalasafaris.myapplication.databinding.ActivityBasicstartBinding
import www.ilalasafaris.myapplication.databinding.SpecieslListBinding


class BasicStart: AppCompatActivity() {
    private var binding: ActivityBasicstartBinding? = null

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityBasicstartBinding.inflate(layoutInflater)
    ...
    private fun initListOfBirds() {
        ...
        binding?.btnSearch?.setOnClickListener {
            val intent = Intent(this, BasicCreate::class.java)
    ...
    private fun setButtonListeners(button: Button, birdNames: List<String>) {
        button.setOnClickListener {
            val intent = when (button) {
                binding?.btn1 -> Intent(this, BasicList1::class.java)
                binding?.btn2 -> Intent(this, BasicList2::class.java)
                binding?.btn3 -> Intent(this, BasicList3::class.java)
                binding?.btn4 -> Intent(this, BasicList4::class.java)
                binding?.btn5 -> Intent(this, BasicList5::class.java)
                binding?.btnAll -> Intent(this, BasicAll::class.java)
    ...
    private fun yourOwnList(birdList: ArrayList<Data>) {
        binding?.btnSave?.setOnClickListener {
            if (birdList.isNotEmpty()) {
                val intent = Intent(this, BasicListSaved::class.java)
                startActivity(intent)
            } else {
    ...
    private fun setupOnBackPressedCallback() {
        onBackPressedDispatcher.addCallback(this@BasicStart, object : OnBackPressedCallback(true) {
            override fun handleOnBackPressed() {
                val intent = Intent(this@BasicStart, MainActivity::class.java)
                startActivity(intent)
                finish()
            }
        })
