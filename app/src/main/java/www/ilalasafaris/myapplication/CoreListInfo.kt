...
import www.ilalasafaris.myapplication.databinding.ActivityCoreinfoBinding
import www.ilalasafaris.myapplication.databinding.SpecieslListBinding

class CoreListInfo : AppCompatActivity() {
                            //Todo 1
    lateinit var binding: ActivityCoreinfoBinding

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
                              //Todo 2
        binding = ActivityCoreinfoBinding.inflate(layoutInflater)
    ...
    private fun initActionBar() {
        val actionbar = supportActionBar
                                //Todo 4
        actionbar!!.title = "Advance List"
        actionbar.setDisplayHomeAsUpEnabled(true)
        actionbar.setHomeButtonEnabled(true)
    }
    private fun setupOnBackPressedCallback() {
                                                         //Todo 5
        onBackPressedDispatcher.addCallback(this@CoreListInfo, object : OnBackPressedCallback(true) {
