...
import www.ilalasafaris.myapplication.databinding.ActivitySavannawestStartBinding
import www.ilalasafaris.myapplication.databinding.SpecieslListBinding

class SavannaWestStart: AppCompatActivity() {

    private var binding: ActivitySavannawestStartBinding? = null

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivitySavannawestStartBinding.inflate(layoutInflater)
    ...
    private fun initBirdDao() {
        ...
        lifecycleScope.launch {
            birdDao.fetchAll_SavannaWest().collect {
    ...
    private fun initListOfBirds() {
        ...
        binding?.btnSearch?.setOnClickListener {
            val intent = Intent(this, SavannaWestCreate::class.java)
    ...
    private fun setButtonListeners(button: Button, birdNames: List<String>) {
        button.setOnClickListener {
            val intent = when (button) {
                binding?.btn1 -> Intent(this, SavannaWestList1::class.java)
                binding?.btn2 -> Intent(this, SavannaWestList2::class.java)
                binding?.btn3 -> Intent(this, SavannaWestList3::class.java)
                binding?.btn4 -> Intent(this, SavannaWestList4::class.java)
                binding?.btn5 -> Intent(this, SavannaWestList5::class.java)
                binding?.btn6 -> Intent(this, SavannaWestList6::class.java)
                binding?.btnAll -> Intent(this, SavannaWestAll::class.java)
    ...
    private fun yourOwnList(birdList: ArrayList<Data>) {
        binding?.btnSave?.setOnClickListener {
            if (birdList.isNotEmpty()) {
                val intent = Intent(this, SavannaWestListSaved::class.java)
                startActivity(intent)
            } else {
    ...
    private fun initActionBar() {
        ...
        actionbar!!.title = "Western Savanna"
    ...
    private fun setupOnBackPressedCallback() {
        onBackPressedDispatcher.addCallback(this@SavannaWestStart, object : OnBackPressedCallback(true) {
            override fun handleOnBackPressed() {
                val intent = Intent(this@SavannaWestStart, MainActivity::class.java)
                startActivity(intent)
                finish()
            }
        })
    }
    @Entity(tableName ="Savanna_West")
    data class Data (
