...
class SavannaEastCreate : AppCompatActivity() {
    ...
    private fun adaptorSearchSaveData() {
        val aabirdname = AdaptorCreate(
            this,
            { showPictureId, showNameid -> displayPicture(showPictureId, showNameid) },
            mediaPlayer,
            mlist
        )
        binding?.lvSaved?.adapter = aabirdname
        binding?.lvSaved?.layoutManager =
        ...
        aabirdname.setOnItemClickListener(object : AdaptorCreate.OnItemClickListener {
            override fun onItemClick(position: Int) {
                ...
                lifecycleScope.launch {
                    birdDao.insert_SavannaEast(
                        SavannaEastStart.Data(
                            idbird = idbird,
                            name = name,
                            sname = sname,
                            picture1 = picture1,
    ...
    private fun iflistisnotfull(birdList: ArrayList<SavannaEastStart.Data>) {
        ...
        binding?.manageList?.setOnClickListener {
            if (birdList.isNotEmpty()) {
                val intent = Intent(this, SavannaEastManage::class.java)
                startActivity(intent)
                MyMediaPlayer.instance!!.onBack()
                finish()
            } else {
    ...
    private fun initActionBar() {
        ...
        actionbar!!.title = "Create List Eastern Savanna"
